...
Programming does has a tendency to over-complicate simple concepts (*foreshadowing... for loops*) but in my experience: **This is the least complicated of the concepts,** in fact... you just have to know like a single table and even so it's really easy if you remember your *less than and greater than signs*

| **Operator / Concept**    | **Syntax** | **Description**                                                 | **Example**                     |
| ------------------------- | ---------- | --------------------------------------------------------------- | ------------------------------- |
| **Equal to**              | `==`       | Checks if two values are equal.                                 | `if (x == 5)`                   |
| **Not equal to**          | `!=`       | Checks if two values are not equal.                             | `if (x != 5)`                   |
| **Greater than**          | `>`        | Checks if the left value is strictly greater than the right.    | `if (score > 90)`               |
| **Less than**             | `<`        | Checks if the left value is strictly less than the right.       | `if (temperature < 0)`          |
| **Greater than or equal** | `>=`       | Checks if the left value is greater than or equal to the right. | `if (age >= 18)`                |
| **Less than or equal**    | `<=`       | Checks if the left value is less than or equal to the right.    | `if (price <= 100)`             |
| **Logical AND**           | `&&`       | Returns true only if **both** conditions are true.              | `if (hasKey && isOpen)`         |
| **Logical OR**            | `\|\|`     | Returns true if **at least one** condition is true.             | `if (isWeekend \|\| isHoliday)` |
| **Logical NOT**           | `!`        | Reverses the boolean state of the condition.                    | `if (!isLoggedIn)`              |
|                           |            |                                                                 |                                 |
***If you understood the syntax, feel free to skip to loops***
